...
## Ключевые выводы
- Recents is a rendered catalog, not a direct listing of ~/.claude/projects/*.jsonl transcripts — it's a hybrid model: embedded claude.ai Code surface (claude.ai/epitaxy) + local session metadata in %APPDATA%\Claude\claude-code-sessions\<account>\<org>\local_*.json + Chromium IndexedDB/Local Storage as a UI cache layer.
...
- claude project purge (dry-run first) removes local project state under projects/, per-session task/debug/file-history data, and the ~/.claude.json project entry — but is NOT a proven fix for stale Desktop Recents since the sidebar doesn't reliably rebuild from transcript files alone.
...
- Backup first (copy, not move) claude-code-sessions, IndexedDB, Local Storage, logs, ~/.claude/projects, history.jsonl, file-history, backups before touching anything.
...
- оригинал: `E:\Obsidian\_originals\deep-research\DR26-06-24-HUB-01-claude-desktop-code-recents-chatgpt.md`
